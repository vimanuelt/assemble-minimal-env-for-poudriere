...
base()
{
  ...
  cp -R ${cwd}/settings/GhostBSD.conf ${release}/usr/local/etc/pkg/repos/
...
ramdisk()
{
  ...
  cd "${cwd}"
  install -o root -g wheel -m 755 "scripts/init.sh.in" "${ramdisk_root}/init.sh"
  sed "s/@VOLUME@/GHOSTBSD/" "scripts/init.sh.in" > "${ramdisk_root}/init.sh"
